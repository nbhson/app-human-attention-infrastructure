...
describe('parsePrUrl', () => {
  ...
  it('rejects a URL with the wrong path shape for its host', () => {
    ...
    expect(() => parsePrUrl('https://github.com/acme/widget')).toThrow(
      /not a GitHub pull-request URL/,
    );
    // A GitLab URL missing the `/-/merge_requests/N` segment.
    expect(() => parsePrUrl('https://gitlab.com/acme/widget/pull/1')).toThrow(
      /not a GitLab merge-request URL/,
    );
  });

  it('rejects a non-numeric PR number', () => {
    expect(() => parsePrUrl('https://github.com/acme/widget/pull/abc')).toThrow(
      /not a GitHub pull-request URL/,
    );
  });
