# HAI Harness — Phase 2: Calibrate & Secure the Review Pipeline
...
**Prerequisite:** Phase 1 complete (`docs/plan/phase-1/`), review loop demonstrable end-to-end, evidence queryable, `v0.1.0-harness` tagged.
...
Phase 1 proved the review loop but is **unmeasured and uncalibrated**: the Attention weights are explicit placeholders, the Context ranker is keyword-only, auth is a single `X-Reviewer-Id` header, and the `AUTO_APPROVABLE` flag nobody acts on.
...
```text
             ...
             │  how well routing/scoring works.            │
             └───────────────────────┬────────────────────┘
                                     ▼
...
By the end of Phase 2, the review pipeline is **measured**: metrics exist for routing precision/recall, Attention weights are fitted from real data, and the evaluation harness can compare two pipeline variants head-to-head (Architecture §24.3 exit criterion).
...
> **Invariant preserved:** still a modular monolith, still Postgres-centric, still `IEventBus`. Phase 2 only widens infrastructure *behind* the seams declared in Phase 1 (`Ranker`, `Retriever`, `Embedder`, `ContentStore`, `LLMProvider`). No engine imports another engine. The AI reviewer stays read-only.
...
| 3 | [day-03.md](day-03.md) | OpenTelemetry — spans across API/engines, trace_id ↔ correlation_id |
...
| 8 | [day-08.md](day-08.md) | Review replay engine — replay a recorded review (Spec 11 §5) |
| 9 | [day-09.md](day-09.md) | A/B shadow harness — run two review-routing variants side-by-side, compare, zero production effect |
...
| 17 | [day-17.md](day-17.md) | Index population — embed sources/diffs; re-embed on change |
...
| 21 | [day-21.md](day-21.md) | Object store (S3/MinIO) — `ContentStore` seam for large diffs (Spec 5 §4.2) |
| 22 | [day-22.md](day-22.md) | Container sandbox for verification (Spec 7 §5.5) |
| 23 | [day-23.md](day-23.md) | Review-report storage + large-diff handling behind `ContentStore` |
...
| 27 | [day-27.md](day-27.md) | E2E under Phase-2 infra — full review pipeline passes with auth + sandbox + metrics |
...
*Prev phase: [Phase 1 — Prove the Review Core Loop](../phase-1/README.md) | Next phase: [Phase 3 — Breadth, Write-back & Close the Loop](../phase-3/README.md)*