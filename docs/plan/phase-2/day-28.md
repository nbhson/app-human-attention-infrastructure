...
| **Spec refs** | Spec 1 §24 (roadmap/exit criteria), Spec 8 (new, Day 24), Spec 10 (new, Day 10); bumped specs: 2, 3, 4, 5, 6, 7, 9, 11 |
...
1. **Version bumps** — every spec that Phase 2 materially changed moves to `v0.3` (the `v0.2` set: Orchestrator, Agent Runtime, Context, Verification, Memory, Evaluation) or gains a `v0.1 → v0.2`/`v0.2` changelog where it only *gained* a section (Artifact §4.2 object store, Attention §4.1 auto-approve). Spec 8 and Spec 10 already exist at v0.1 (written Days 10/24).
...
3. An updated **dev guide** reflecting the new infra (auth, observability, sandbox, object store, cache, eval) and the wiring map's R7–R12 boundary rules.
...
| 3 Agent Runtime | §6.1 replay (Day 8), §14.3 sandbox Code Mode (Day 23) | v0.3 |
...
| 9 Memory Evidence | §3.1 evidence now includes `code_mode_sessions` + `content_hash` | v0.3 |
| 11 Evaluation | §4 metrics realized (Days 6–7), §5 shadow harness (Day 9), §6 closed loop | v0.3 |
...
|------|-------------|
| `docs/core/{2,3,4,7,9,11}_*.md` → v0.3 | Bumped + changelog |
...
- [ ] `grep -L "## Changelog"` over `docs/core/*.md` for the *Phase-2-touched* specs is empty (every touched spec has one).
...
- **Version numbers only matter if they're honest.** Bumping everything to v0.3 "for consistency" makes v0.3 mean nothing. Bump only material changes; an untouched spec that got bumped is a lie a future reader will uncover mid-incident.
