...
| **Spec refs** | Spec 8 (new standalone); Spec 6 §4.1 (alert fatigue / review load), Spec 6 §4 (decision records); Phase-1 `@harness/review` backend (Day 22 of Phase 1) |
| **Estimated effort** | 6 hours |
| **Prerequisites** | Day 23 (OPERATOR-tier approvals); Phase-1 review backend (queue, `review.decision_submitted`, dwell metrics from Day 4) |
...
1. A **standalone Spec 8 — Human Review Interface** (`docs/core/8_Human_Review_Interface_v0.1.md`) that consolidates the review surface, which until now lived *implicitly* across the Phase-1 `@harness/review` backend, the attention engine's escalation path (Spec 6), and Day-23's OPERATOR-tier approvals.
...
3. The **OPERATOR/REVIEWER role+permission split** from Days 1–2 reconciled with the interface's write operations (who can claim, decide, escalate, and — new — *approve tier-2 tool calls*).
4. A **front-end interface spec** (not implementation): the actions a reviewer can take, the data each queue item must show (change diff, attention assessment, verification evidence, calibration confidence), and the dwell/decision telemetry each action emits.
...
| Sandboxed tier-2 approval surface | **Spec 8** (approve/deny action) | Spec 3 §14.1 / Day 23 |
...
- **Actions**: `claim`, `decide(accept|reject|escalate)`, `approve_tool(tier2)`, `release`. Each spec'd with required role, allowed states, and the event it emits.
- **Display contract**: a queue item must render change diff (`changes`), attention assessment (`priority` + `label`), verification evidence (`verification_reports`), and calibration confidence (Day 12); missing evidence must be *shown as missing*, not omitted.
...
- [ ] Add the review queue route + actions (claim/decide/release/approve_tool) to the React app, wired to the spec'd contract — read-only prototype today; no new backend behavior.
...
- [ ] `docs/core/8_Human_Review_Interface_v0.1.md` exists with the §2 state machine, roles, action/event list, decision schema, and telemetry contract.
...
- [ ] The review queue route exercises claim/decide/release/approve_tool via the spec'd interface only.
...
- **Next (Day 25):** Week 5 checkpoint — sandbox + object store + cache integrated, shadow metrics in the report.

---

*Prev: [Day 23 — Container Sandbox for Agent Code Mode](day-23.md) | Next: [Day 25 — Week 5 Checkpoint](day-25.md)*