...
_The reference run for the Week-6 E2E. A benchmark, not a snapshot of perfection:
...
because it is also _observable_: the trace maps back to the task, the decision is
...
context cache _moved_. `sandboxRun = 0` is expected here and documented, not a
...
- **Trace ids are not stable values.** They change every run; what must stay stable
  is the _existence_ of the `trace_correlation` mapping, which the reconstruct
  assertion enforces.

---

_Green gate: `pnpm lint`, `pnpm -r typecheck`, `pnpm -r test` (673 tests, 129
