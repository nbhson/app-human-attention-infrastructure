...
*Day-30 checkpoint (Phase 2). Week 6 was the hardening week that ended in the
...
honest by design, numbers-first, blameless — and green before committed.*
...
- **The shadow discipline held all the way to the exit.** The single most
  load-bearing fact of the phase — semantic retrieval was *installed* for three
  weeks and *never* became the default — was re-proven at the end, not just
  ...
  zero embedder calls. Day 29's dry-run then *used* that shadow the right way:
...
- **The A/B harness caught a real disagreement, and refused to over-claim it.**
  ...
  consumption is fixed. The recommendation — *"promote semantic ranking to a
  real A/B, collect live outcome data first"* — is the honest answer, not a soft
...
- **The weight fit did not earn the default — and nothing forced it to.**
  ...
  *aspiration* ("calibrated weights by end of phase"), caught by the governance
...
- **The canonical A/B fixture was degenerate, and that was caught before it
  ...
  the harness honestly *skips* the single-file tau rather than padding it to 0.
...
- **Escalation leakage is 1.0.** On the N=4 window every auto-approvable item
  ...
  "learn and automate under guardrails" premise is unfunded. *This is the number
  to watch.*
...
- **The routing metrics will outgrow their baseline.** Precision 0.333 / recall
  ...
  Phase 3 must commit to the numbers-checkpoint habit so the *next* exit review
