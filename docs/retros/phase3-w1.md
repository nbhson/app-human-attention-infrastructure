...
*Day-05 checkpoint (Phase 3, Week 1). Week 1 built the connectivity layer that
...
- **"One config connects any tool" is proven, not asserted.** The week's whole
  ...
  The stub is a *single* stdio subprocess answered by tool name
...
- **The seam stayed, the transport changed.** `MCPGitProvider` and
  `MCPTicketProvider` return the *same* `PullRequest`/`Issue` value objects the
...
- **Day-04's write primitives shipped once as "not wired", then were rewired.**
  ...
  showed the plan wants the ticket write path *wired as MCP tools* (fetch +
...
- **The demo lives in the api `scripts/` dir, not the root `scripts/demo/`.**
  The root `scripts/demo/` holds markdown *runbooks* (week1…week6); the Day-05
...
- **`WriteBackService` (Day 06) is the first *write* over the same MCP tools.**
  ...
  it safe to *never* write externally by default — watch that the off-by-default
  posture is asserted in a test, not just documented.
- **`--live` is an explicit refusal, not a silent fallback.** The demo refuses to
  run live when `mcp.config.json` is absent/empty; it will *never* fetch through
...
*Checkpoint rule applied: `pnpm typecheck` (42/42), `pnpm lint`, and `pnpm test`
