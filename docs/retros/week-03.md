...
_Day-15 checkpoint (Phase 2). Third pass, over the calibration + gated auto-approve
...
so plainly, and documents why that is the correct outcome to ship._
...
- **Day 14 — auto-approve flag + kill-switch + sampling audit.** An
  ...
  (calibration _green_ ∧ flag _on_ ∧ item _under the bar_), a machine decision
...
is on the record: _"fitted weights did not beat the Phase-1 placeholder on
held-out validation; the placeholder stays active."_

**The one-line verdict:** _No — calibration did not improve routing on the
...
stays active and auto-approve stays red._

That verdict needs the honest caveat it is owed. This is not evidence that
calibration _cannot_ help — it is evidence that **the corpus is not yet large
...
right thing: it measured a worse loss and _declined to promote itself_. The
...
- **A benchmark corpus and gold labels.** Same wall as Week 2, now with a sharper
  edge: the fitter _works_, but 5 rows (1 held-out) is not a population, and
  ...
  be validated as an _improvement_ — only as a change with a measured direction.
- **The A/B seam is unblocked, but unused.** Day-09's shadow harness can compare
  ...
  that should be built to _enable_ that comparison is the semantic ranker itself —
...
- **The sampling audit is a canary, not a net.** 10% of approvals get a silent
  ...
  residual risk is carried until the corpus is large enough to _prove_ the
...
- **Keep the placeholder; do not tune by hand to make the fit look better.** The
  honest result of a fit that loses is to _keep_ the placeholder, exactly as the
...
_Checkpoint rule applied: `pnpm eval:make-dataset --label=outcome` printed a real
...
asserted by `packages/di/src/__tests__/architecture.test.ts`._
