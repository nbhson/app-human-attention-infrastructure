...
- **Tracker owns everything *before* merge.** Content (content-addressed
  ...
  source of truth.
- **Git owns everything *after* merge.** The repository history, the actual
...
- **Append-only provenance.** Tracker metadata (`artifacts`, `changes`,
  ...
  Only snapshot *content* may be pruned by a future retention job.
