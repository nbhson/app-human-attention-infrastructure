...
| `ReviewIngestService`       | `ReviewIngestService(db, bus, taskService, gitProvider, ticketProvider, reviewAgent, aiProvider, model, logger, memoryProvider, maxBatchSize, maxBatchTokens, twoPassEnabled)` | review-reorient                                       | `routes/reviews.ts` (`POST /api/reviews`, `GET /api/reviews/:id`); async worker (`ReviewWorkerSubscriber` calls `processReview`)                                                                                                                                                                                                                                                                                                 |
| `ReviewWorkerSubscriber`    | `ReviewWorkerSubscriber(ReviewIngestService, EventBus, Logger)`                                                                                                                | review-reorient Phase 4                               | eager-resolved at boot (side effect: subscribes to `review.requested` → calls `ReviewIngestService.processReview` asynchronously)                                                                                                                                                                                                                                                                                                |
...
| `TargetedVerifier` | `verification-engine/src/targeted-verifier.ts` (day-14) | The _routing policy_: affected set when complete and non-empty, else full suite                                                                               | The app injects `resolveAffected = (changed) => affectedTests(changed, graph)` — the engine owns the policy, the leaf owns the graph; neither imports the other.                                                   |
...
proves targeted verdict == full verdict on every case (green _and_ red) with fewer
...
_selectable_ per request via an explicit `rank_method`, and flipping the default is a
...
_often_ surfaced") with a **learned** signal ("did surfacing it _help_?").
...
Day-34 adds a **durable `IEventBus`** as an _optional_ swap behind the exact same
...
inflation/false-pass) are _set_ by `@harness/evaluation` on Day 06, never
...
`applyGauges` writes to _its own_ process's register, which nobody scrapes — the
...
ReviewVerificationService   MemoryIngestor   ReviewWorkerSubscriber
```
...
49. `ReviewWorkerSubscriber` — needs `ReviewIngestService`, `EventBus`, `Logger`; subscribes on `review.requested` → calls `processReview` in background (Phase 4; eager-resolved).
50. `MetricsComputer` — no deps.
51. `Orchestrator` / `AgentRuntime` / `AttentionEngine` — stubs (see above).
...
memory. If a name below is wrong, the _source_ moved — fix the source, then this.
...
the other 51 are the real registrations. Note there is **no `TOKENS.Runner`** — the
...
review.requested              integration.writeback_completed
...
Review                review_queue, decisions, review_decisions,
                      review_reports (review_status, batch_progress), review_findings,
                      review_verifications, fix_suggestions
