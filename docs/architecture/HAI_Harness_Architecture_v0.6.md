...
- **Phase-4 async + tag polish** (`v0.5.0-harness` → `v0.6.0-harness`): No new product surface beyond the async pipeline above; the two tags record the `ReviewWorkerSubscriber` eager wiring (`bootstrap.ts:859` — 13→14 eager tokens) and doc/badge polish (route counts, env drift, table counts). Functional delta vs `v0.4.0` is nil — the review slice is frozen.
