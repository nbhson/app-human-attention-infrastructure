...
| **Review modes**       | Two modes via Triage Rules settings (`POST /api/triage-rules`):<br>- **High-signal** (default, OFF): only CRITICAL/MAJOR findings — attention router for humans<br>- **Full code review** (ON): ALL severities (CRITICAL/MAJOR/MINOR/NIT/INFO) — like GitHub Copilot Review, SonarQube                                                          |
